...
zokou({
  ...
  try {
    ...
    if (videos && videos.length > 0 && videos[0]) {
      const urlElement = videos[0].url;
      const apiResponse = await fetch(`http://api.giftedtechnexus.co.ke/api/ytmp3?url=${encodeURIComponent(urlElement)}`);
...
zokou({
  ...
  try {
    ...
    if (videos && videos.length > 0 && videos[0]) {
      const Element = videos[0];
      const apiResponse = await fetch(`http://api.giftedtechnexus.co.ke/api/ytmp3?url=${encodeURIComponent(Element.url)}`);
      const apiResult = await apiResponse.json();
...
reaction: "💿"
 ...
 if (!arg[0]) {
  ...
  try {
    ...
    const urlYt = videos[0].url;
    const apiResponse = await fetch(`http://api.giftedtechnexus.co.ke/api/ytmp3?url=${encodeURIComponent(urlYt)}`);
    const apiResult = await apiResponse.json();

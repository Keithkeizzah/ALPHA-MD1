...
keith({
  ...
  // Fetch existing alive data
  const data = await getDataFromAlive();

  // If no arguments, display current alive data
  if (!arg || !arg[0] || arg.join('') === '') {
    if (data) {
      ...
      // Check if the link is a video (mp4, gif)
...
*Powered by keithkeizzah*`
      ...
      // Check if the link is an image (jpeg, png, jpg)
...
*Powered by keithkeizzah*`
      ...
      // If it's neither a video nor an image, just send the text
      else {
        repondre(`
*Owner* : ${s.OWNER_NAME}
*Mode* : ${mode}
...
*Powered by keithkeizzah*`);
      ...
      // Handle case when no alive data exists
      if (!superUser) {
        repondre("There is no alive data for this bot, idiot.");
        return;
      }
      await repondre("You have not yet saved your alive data. To do this, enter: .alive message;lien");
      repondre("Don't do fake things :)");
    }
  } else {
    // If arguments are provided, update the alive data
    if (!superUser) {
    ...
    if (!texte || !tlien) {
      repondre("Invalid format! Use: .alive message;lien");
      return;
    }

    // Update or add the new alive message and link

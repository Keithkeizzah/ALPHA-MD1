...
keith({
  ...
  try {
    ...
    if (audioResults && audioResults.videos.length > 0) {
      ...
      const response = await fetch(`https://apis.ibrahimadams.us.kg/api/download/ytmp3?url=${encodeURIComponent(audioUrl)}&apikey=cracker`);
...
keith({
  ...
  try {
    ...
    if (videoResults && videoResults.videos.length > 0) {
      ...
      const response = await fetch(`https://apis.ibrahimadams.us.kg/api/download/ytmp4?url=${encodeURIComponent(audioUrl)}&apikey=cracker`);
      const jsonResponse = await response.json();
